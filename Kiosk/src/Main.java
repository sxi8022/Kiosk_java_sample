...
public class Main {
    ...
    public void displayMenu() {
        ...
        for(int i = 0 ; i < og.size(); i ++) {
            System.out.println(
                    og.get(i).MenuNo
                            + ". "
                            +  String.format("%-10s", og.get(i).Name)
                            + " | "
                            + og.get(i).Desc);
        }

        System.out.println("종료하려면 -1을 입력하세요.(상품조회 : 0)");
    }
    ...
    public void selectMenu() {
        ...
        if (selectNo < 0 || selectNo > (og.size() + cg.size())) {
            ...
            T.initProduct(selectNo);
            T.selectProduct(selectNo);
        }
    }
    ...
    public Product chooseOption(int selectMenu, int selectNo) {
        ...
        if (selectMenu == 1) {
            if (selectNo == 5) {
                ...
                System.out.println(". Single(W " + origin.Price + ")        2. Double(W 7.4)?");
